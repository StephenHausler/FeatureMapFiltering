...
function [template] = CNN_Create_Template_Filter(net,Im,actLayer1,finalMaps)
...
for j = 1:length(finalMaps)
    k = finalMaps(j);
    sum_array(1,j) = max(max(act1(:,:,1,k)));
    sum_array(2,j) = max(max(act1(1:sh11,1:sh12,1,k)));
    sum_array(3,j) = max(max(act1(1:sh11,sh12:sz1(2),1,k)));        
    sum_array(4,j) = max(max(act1(sh11:sz1(1),1:sh12,1,k)));
    sum_array(5,j) = max(max(act1(sh11:sz1(1),sh12:sz1(2),1,k)));
end
